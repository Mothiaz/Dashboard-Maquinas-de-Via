################ DROPDOWNS ####################

NATION_DROPDOWN = 'nation-dropdown'
DATE_DROPDOWN = 'date-dropdown'
YEAR_DROPDOWN = 'year-dropdown'
MONTH_DROPDOWN = 'month-dropdown'
SYSTEM_DROPDOWN = 'system-dropdown'
FLEET_DROPDOWN = 'fleet-dropdown'
EQUIPMENT_DROPDOWN = 'equipment-dropdown'
EQUIPMENT_TYPE = 'equipment-type'
EVENT_TYPE = 'event-type'
SET_DROPDOWN = 'set-dropdown'
ITEM_DROPDOWN = 'item-dropdown'
PROBLEM_DROPDOWN = 'problem-dropdown'

################ BUTTONS ####################

SELECT_ALL_YEARS_BUTTON = 'select-all-years-button'
SELECT_ALL_MONTH_BUTTON = 'select-all-month-button'
SELECT_ALL_DATES_BUTTON = 'select-all-dates-button'

SELECT_ALL_FLEETS_BUTTON = 'select-all-fleets-button'
SELECT_NULL_FLEETS_BUTTON = 'select-null-fleets-button'

SELECT_ALL_EQUIPS_BUTTON = 'select-all-equips-button'
SELECT_NULL_EQUIPS_BUTTON = 'select-null-equips-button'

SELECT_ALL_SYSTEMS_BUTTON = 'select-all-systems-button'
SELECT_NULL_SYSTEMS_BUTTON = 'select-null-systems-button'

SELECT_ALL_SETS_BUTTON = 'select-all-sets-button'
SELECT_NULL_SETS_BUTTON = 'select-null-sets-button'

SELECT_ALL_ITEMS_BUTTON = 'select-all-items-button'
SELECT_NULL_ITEMS_BUTTON = 'select-null-items-button'

SELECT_ALL_PROBLEMS_BUTTON = 'select-all-problems-button'
SELECT_NULL_PROBLEMS_BUTTON = 'select-null-problems-button'

################ RANGES ####################

DATE_RANGE = 'date-range'

################ GRAPHS ####################

FLEET_BAR_CHART = 'fleet-bar-chart'
EQUIPMENT_BAR_CHART = 'equipment-bar-chart'
SYSTEM_BAR_CHART = 'system-bar-chart'
SET_BAR_CHART = 'set-bar-chart'
ITEM_BAR_CHART = 'item-bar-chart'
PROBLEM_BAR_CHART = 'problem-bar-chart'
TIME_LINE_CHART = 'time-line-chart'
