...
def load_data(path: str) -> pd.DataFrame:
    ...
    data.rename(columns={'equip.': DataSchema.EQUIPMENT, 'retenção': DataSchema.REPAIRTIME,
                         ...
                         'tipo evento': DataSchema.EVENT},
                inplace=True)  # renomeia colunas
    data = data.iloc[:, 0:19]  # seleciona as primeiras 19 colunas: ate 'problema'
    data.drop(columns=['ano', 'mês', 'falha'], inplace=True)  # dropa colunas inuteis
